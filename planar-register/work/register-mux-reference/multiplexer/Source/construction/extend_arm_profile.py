...
base=solid(trimesh.load(D/'Reinforced fork candidate.stl'))
...
p=D/'Uniform arm candidate.stl';t=clean(mesh(s));t.export(p,file_type='stl_ascii');t=clean(trimesh.load(p));assert t.is_watertight and len(t.split())==1;t.export(p,file_type='stl_ascii')
t.apply_transform(trimesh.transformations.rotation_matrix(-np.pi/2,[0,1,0]));t.apply_translation(-t.bounds[0]);t.export(D/'Uniform arm candidate - print.stl',file_type='stl_ascii');mask=(t.face_normals[:,2]<-.72)&(t.triangles_center[:,2]>.05)
