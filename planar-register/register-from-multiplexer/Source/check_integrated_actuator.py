...
R=Path(__file__).resolve().parents[1];O=R.parent/'work/integrated-cam-development';B=R.parent/'work/register-mux-reference/multiplexer';trace=json.loads((B/'Switching trace.json').read_text())['frames']
...
for bank in ['Memory','Write']:
 old=trimesh.load(B/'Carriage fork and roof.stl');lever=trimesh.load(B/'Short lever.stl');pivot=np.array([13.192323604,10.2,32.128448698]);shift=np.array([0,0,0]) if bank=='Memory' else np.array([-85.0,0,16]);old.apply_translation(shift);lever.apply_translation(shift);pivot+=shift
 ...
 cut=m.Manifold.cube([250,30,140]).translate([-140,12.1 if bank=='Memory' else 18.9,-30]);added=tm(delta^cut);ca=trimesh.collision.CollisionManager();cl=trimesh.collision.CollisionManager();ca.add_object('addition',added);cl.add_object('lever',lever);hits=[]
