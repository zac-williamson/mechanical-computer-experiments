...
def lift(q):
 ...
 return max(0,z-53.3)
hits={}
for qm,qe in itertools.product(np.linspace(-3.75,3.75,41),repeat=2):
 shapes={};bounds={};up=max(lift(qe),3.2 if abs(qm)<3.0 else 0)
 for p in meta:
...
print(json.dumps(hits,indent=2));(O/'Printed travel intersections.json').write_text(json.dumps(hits,indent=2))
