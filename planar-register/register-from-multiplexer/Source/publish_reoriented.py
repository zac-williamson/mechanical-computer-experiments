...
R=Path(__file__).resolve().parents[1];B=R.parent/'work/register-mux-reference/multiplexer';O=R/'Planar register'
...
def segment(m0,m1,e0,e1,label):
 for t in np.linspace(0,1,100):
  qm=m0+(m1-m0)*float(t);qe=e0+(e1-e0)*float(t);cmd=2*(qe-4.3);trace.append(dict(q=qm,qm=qm,qe=qe,s=min(0,cmd),angle=float(np.arcsin(qe/10)),bm=beta(qm),be=beta(qe),b=0,w=0,g=0,segment=label))
...
js=js[:start]+'''let q=p.bank==='Memory'?f.qm:f.qe,bankSign=1;
...
if(mo==='bell-crank')[R,t]=rot('Y',f.angle,[20,0,-52.8]);
if(mo==='rocker'&&p.bank==='Memory')[R,t]=rot('Y',f.bm*Math.PI/180,[13.192323604,10.2,32.128448698]);
if(mo==='rocker'&&p.bank==='Write')[R,t]=rot('Y',f.be*Math.PI/180,[109.992323604,10.2,48.128448698]);
''' +js[end:]
