...
states=[]
for k,e in itertools.product([0,1],repeat=2):
 for w in [0,1]:states.append(dict(K=ends[k],W=ends[w],E=ends[e]))
for i in [150,300,450,600,750,900,1050,1200,1350,1500,1650]:states.append(dict(K=trace[i],W=ends[0],E=ends[0]))
for i in [150,450,900,1200,1500]:states.append(dict(K=ends[0],W=trace[i],E=ends[0]))
