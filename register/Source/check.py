...
import trimesh
ROOT=Path(__file__).resolve().parent;import os;OUT=Path(os.environ.get('REGISTER_OUTPUT',str(ROOT.parent)));D=json.loads((OUT/'Assembly manifest.json').read_text())
...
for k in [0,1]:
 for e in [0,1]:
  for p in pts[::30]:poses.append({'K':ends[k],'W':p,'E':ends[e]})
for p in pts[::15]:poses.append({'K':p,'W':ends[0],'E':ends[0]})
for k in [0,1]:
 for p in pts[::30]:poses.append({'K':ends[k],'W':ends[1],'E':p})
