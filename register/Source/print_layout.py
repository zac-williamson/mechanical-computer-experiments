...
for p in D['prints']:
 ...
 if p['bores']:
  ...
  entries=[np.array([lo[0],10.2+d[1],z+d[2]]) for z in [24,32,40]]
 elif 'Bearing wall' in n:entries=[np.array([lo[0],10.2+d[1],z+d[2]]) for z in [24,32,40]]
 ...
 elif 'Direct lever' in n:entries=[d+np.array([14.500924592586399,-3.261892187851191,24])]
 if p['actor']=='W':
  if p['bores']:
   entries=[]
   for c in p['bores']:
   ...
   base=n[2:];orig=trimesh.load(ROOT/'Inputs'/(base+'.stl'));ol=orig.bounds[0];local=[]
   if 'carriage half' in n:local=[[ol[0],10.2,z] for z in [24,32,40]]
   elif 'Bearing wall' in n:local=[[ol[0],10.2,z] for z in [24,32,40]]
   ...
   elif 'Direct lever' in n:local=[[14.500924592586399,-3.261892187851191,24]]
