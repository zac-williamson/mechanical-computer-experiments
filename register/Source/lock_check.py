...
for q in np.linspace(-4.35,4.325,71):
 x=-7+q;good=[]
 for z in np.arange(14.5,18.401,.01):
 ...
 rows.append(dict(q=float(q),withdrawal_from_hold=float(q+4.35),min_follower_Z=min(good),max_follower_Z=max(good),min_toe_engagement_mm=min(good)+14-30,max_toe_engagement_mm=max(good)+14-30))
