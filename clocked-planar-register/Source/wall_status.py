...
with (O/'Model inventory.csv').open('w',newline='') as f:
 w=csv.writer(f);w.writerow(['instance','module','kind','native_part_or_inherited_source','one_bit_quantity','eight_bit_quantity','development_stl'])
 for p in ps:
...
for name,item in reports.items():
 ...
 if item['current']:
  ...
  elif name=='Bearing attachment checks':status=f"{'PASS' if r['attachment_pass'] else 'FAIL'}: {len(r['removable_transmission_walls'])} removable transmission walls and four cheeks; two engaged pin axes each; two-piece frame joint verified; {len(r.get('removable_frame_fixtures',[]))} paired-pin fixture mounts"
