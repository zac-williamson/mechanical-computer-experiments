...
from compact_pose import joint,transform
...
assert trace['geometry_sha256']==digest and trace['contact_pose_pass'], 'Stale or unresolved contact trace'
frames=[f for c in trace['cases'] for f in c.get('frames',[])]
def solid(a):
...
for fi,f in enumerate(frames[::12]):
 ...
 for (p,s),d in zip(native,nm):
  ...
  for q,t,qt in pp:
   ...
   if vol>.001:
    pair=(p['id'],q['id'])
    if pair not in hits or hits[pair]['volume_mm3']<vol:hits[pair]=dict(native=pair[0],printed=pair[1],volume_mm3=vol,sampled_frame=fi*12)
result=dict(scope=__doc__,geometry_sha256=digest,frame_stride=12,unique_poses=count,narrow_checks=checks,potential_collisions=sorted(hits.values(),key=lambda h:-h['volume_mm3']),mechanically_qualified=False)
