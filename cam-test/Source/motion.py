...
TURN=np.diag([-1.,1.,-1.])
def lift(q):return -5.2*np.clip((q+1.85)/2.35,0,1)
def transform(actor,motion,st,actors):
 T=np.eye(4);d=np.array(actors.get(actor,[0,0,0]));p=st.get(actor,dict(q=0,b=0,w=0,g=0,offset=0));q=p['q'];R=TURN if actor=='W' else np.eye(3)
