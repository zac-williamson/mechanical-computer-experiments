...
states=[dict(K=ends[k],W=ends[w]) for k,w in itertools.product([0,1],repeat=2)]
for q in [-3,-1,0,1,3]:
 mid=min(trace,key=lambda p:abs(p['q']-q))
 states.append(dict(K=mid,W=ends[0]))
 for k in [0,1]:states.append(dict(K=ends[k],W=mid))
