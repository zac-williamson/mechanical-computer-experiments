...
ROOT=Path(__file__).resolve().parent;import os;OUT=Path(os.environ.get('REGISTER_OUTPUT',str(ROOT.parent)));D=json.loads((OUT/'Assembly manifest.json').read_text());P=OUT/'Print parts';P.mkdir(exist_ok=True)
parts=[];checks=[]
for p in D['prints']:
 t=trimesh.load(OUT/p['path']);R=np.asarray(p['rotation']);tt=t.copy();tt.vertices=tt.vertices@R.T;shift=-tt.bounds[0];tt.vertices+=shift;tt.export(P/(p['id']+'.stl'))
 tri=tt.triangles;bed=np.all(np.abs(tri[:,:,2])<1e-4,axis=1);area=float(tt.area_faces[bed].sum());assert area>5,(p['id'],area);assert tt.is_watertight and tt.is_winding_consistent
 # Explicit bearing entrances, including the inherited actuator walls.
 entries=[];n=p['id'];d=np.array(D['actors'].get(p['actor'],[0,0,0]));lo=t.bounds[0];hi=t.bounds[1]
 if p['bores']:
  for c in p['bores']:
   c=np.array(c,dtype=float);axis=0 if 'bearing' in n else 2;c[axis]=lo[axis];entries.append(c)
 elif 'carriage half' in n:
  entries=[np.array([hi[0] if 'Right carriage' in n else lo[0],10.2+d[1],z+d[2]]) for z in [24,32,40]]
 elif 'Bearing wall' in n:entries=[np.array([hi[0] if 'Right carriage' in n else lo[0],10.2+d[1],z+d[2]]) for z in [24,32,40]]
 elif 'actuator bridge' in n or 'Rear bridge' in n:
  entries=[d+np.array([x,y,54.8 if 'Rear bridge' in n else 19.8]) for x,y in [(0,2.2),(14.500924592586399,-3.261892187851191)]]
 elif 'Direct lever' in n:entries=[d+np.array([14.500924592586399,-3.261892187851191,20.2])]
 if p['actor']=='W':
  if p['bores']:
   entries=[]
   for c in p['bores']:
    c=np.array(c,dtype=float);c[0]=hi[0];entries.append(c)
  else:
   base=n[2:];orig=trimesh.load(ROOT.parents[1]/'multiplexer'/(base+'.stl'));ol=orig.bounds[0];local=[]
   if 'carriage half' in n:local=[[orig.bounds[1,0] if 'Right carriage' in n else ol[0],10.2,z] for z in [24,32,40]]
   elif 'Bearing wall' in n:local=[[orig.bounds[1,0] if 'Right carriage' in n else ol[0],10.2,z] for z in [24,32,40]]
   ...
   elif 'Direct lever' in n:local=[[14.500924592586399,-3.261892187851191,20.2]]
   entries=[np.array(c)*[-1,1,-1]+d for c in local]
 heights=[float((c@R.T+shift)[2]) for c in entries];assert all(abs(z)<1e-3 for z in heights),(n,heights)
 checks.append(dict(part=n,file='Print parts/'+n+'.stl',watertight=True,consistent_winding=True,shells=len(tt.split()),size_mm=tt.extents.tolist(),bed_contact_area_mm2=area,bearing_entrance_heights_mm=heights,rotation=R.tolist(),translation=shift.tolist(),notes=p['sliding']))
