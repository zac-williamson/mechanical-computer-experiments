...
lib=LDraw(str(Path(os.environ.get('LDRAW_PATH','/Applications/Studio 2.0/ldraw'))/'parts/23948.dat'));trace=json.loads(re.search(r'const POSES=(\[.*?\]);',(ROOT/'Sources/Inputs/multiplexer/Viewer.html').read_text(),re.S)[1]);ends={str(k):min(trace,key=lambda p:abs(p['q']-(4.325 if k==0 else -4.35))) for k in [0,1]}
...
for ac,off in D['actors'].items():
 a=np.array([36.25231148,-14.70473047]);pivot=np.array([14.50092459,-3.26189219]);b=np.array([21.75138689,-7.94283828]);ang=math.radians(ends['0']['b']);R=np.array([[math.cos(ang),-math.sin(ang)],[math.sin(ang),math.cos(ang)]]);b=pivot+R@(b-pivot);direction=math.atan2(b[1]-a[1],b[0]-a[0]);polys=[]
